...
class BaseTransformer:
    ...
    @staticmethod
    def _set_columns_data(data_list, columns_data_list, columns):
        if columns_data_list is None:
            return

        if isinstance(columns_data_list[0], (pd.DataFrame, pd.Series)):
            for i in range(len(data_list)):
                columns_data_list[i].index = data_list[0].index

        for i in range(len(data_list)):
            if len(columns_data_list[i].shape) == 1:
                data_list[i][columns[0]] = columns_data_list[i]
            else:
                data_list[i][columns] = columns_data_list[i]

    @staticmethod
    def _set_columns_sample_data(data, columns_data, columns):
    ...
    def fit(self, data_list, columns):
        ...
        """
        self._store_columns(columns, data_list[0])

        columns_data_list = []
        for i in range(len(data_list)):
            columns_data_list.append(self._get_columns_data(data_list[i], self.columns))
        self._fit(columns_data_list)

        self._build_output_columns(data_list[0])
    ...
    def transform(self, data_list, drop=True):
        ...
        if any(column not in data_list[0].columns for column in self.columns):
            return data_list
        ...
        for i in range(len(data_list)):
            columns_data_list.append(self._get_columns_data(data_list[i], self.columns))
        transformed_data_list = self._transform(columns_data_list)

        self._set_columns_data(data_list, transformed_data_list, self.output_columns)
        if drop:
            pass
    ...
    def reverse_transform(self, data, drop=True):
        ...
        self._set_columns_sample_data(data, reversed_data, self.columns)
        if drop:
