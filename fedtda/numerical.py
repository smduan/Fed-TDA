...
from fedtda.fed_compute import fed_min, fed_max
...
class NumericalTransformer(BaseTransformer):
    ...
    def _fit(self, data_list):
        ...
        self._min_value = fed_min(data_list) if self.min_value == 'auto' else self.min_value
        self._max_value = fed_max(data_list) if self.max_value == 'auto' else self.max_value
        ...
        self.null_transformer.fit(data_list[0])
    ...
    def _transform(self, data_list):
        ...
        Returns:
            numpy.ndarray
        """
        return [self.null_transformer.transform(data) for data in data_list]
