...
class HyperTransformer:
    ...
    def _fit_field_transformer(self, data_list, field, transformer):
        ...
        transformer.fit(data_list, field)
    ...
    def fit(self, data_list):
        ...
        self._input_columns = list(data_example.columns)
        self._populate_field_data_types(data_example)

        # Loop through field_transformers that are first level
        for field in self.field_transformers:
            if self._field_in_data(field, data_example):
                data_list = self._fit_field_transformer(data_list, field, self.field_transformers[field])
    ...
    def transform(self, data_list):
        ...
        # unknown_columns = self._subset(data_list[0].columns, self._input_columns, not_in=True)
        # if self.copy:
        #     data = data.copy()

        for transformer in self._transformers_sequence:
            data_list = transformer.transform(data_list, drop=False)

        transformed_columns = self._subset(self._output_columns, data_list[0].columns)
        for i in range(len(data_list)):
            data_list[i] = data_list[i].reindex(columns=transformed_columns)
